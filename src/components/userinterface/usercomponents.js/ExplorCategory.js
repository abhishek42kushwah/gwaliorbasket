...
export default function ExplorCategory(){
  ...
  { return categoryList.map((item)=>{
        return(<div onClick={()=>handleClick(item.categoryid)}
         style={{width:'10%',
       padding:3,
       marginBottom:15,
       background:"#f7eaf9",
       borderRadius:'10%',
        ...
        <div style={{color:'#792c85',fontFamily:'Poppins',marginTop:40,fontSize:xs?4:sm?6:md?8:lg?16:16,fontWeight:'bolder',textAlign:'center'}}>{item.categoryname}</div>
         <img src={`${ServerURL}/images/${item.icon}`} style={{width:'70%',margin:10,marginTop:25}}/></div>)
  })
  } 
